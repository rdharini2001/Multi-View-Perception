...
class Annotator:
    ...
    def kpts(self, kpts, shape=(640, 640), radius=5, kpt_line=True):
        ...
        for i, k in enumerate(kpts):
            ...
            print(f"Keypoint {i}: ({int(k[0])}, {int(k[1])})")
            self.im = cv2.putText(self.im, f"{i}", (int(k[0]), int(k[1])), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (255, 0, 0), 1, cv2.LINE_AA)
            ...
            if len(kpts):
                ...
                for j in range(len(kpts_)):
                    if labels or conf[j] > 0.25:  # 0.25 conf thresh
                        ...
                        for i, kp in enumerate(kpts_):
                          x = int(kp[0])
                          y = int(kp[1])
                          print(annotator.text((x, y), str(i)))
